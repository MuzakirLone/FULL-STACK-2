...
def test_get_student(client):
    ...
    response = client.get("/students/{}".format(student_id))
    assert response.status_code == 200
...
def test_update_student(client):
    ...
    response = client.put("/students/{}".format(student_id), json={"name": "Student-3 Updated"})
    assert response.status_code == 200
...
def test_delete_student(client):
    ...
    response = client.delete("/students/{}".format(student_id))
    assert response.status_code == 200

    # Verify the student has been deleted
    get_response = client.get("/students/{}".format(student_id))
